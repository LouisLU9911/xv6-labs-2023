...
e1000_transmit(struct mbuf *m)
{
  ...
  acquire(&e1000_lock);
  while (regs[E1000_TDH] != regs[E1000_TDT]) {
    // while tx_ring is not empty
    if (tx_ring[regs[E1000_TDH]].status & E1000_TXD_STAT_DD) {
      // free TX Descriptor Head when it is done
      mbuffree(tx_mbufs[regs[E1000_TDH]]);
      tx_mbufs[regs[E1000_TDH]] = 0;
  ...
  // create a new mbuf at TX Descriptor Tail
